...
import string
import argparse
from vn_norm.text.rules import *
from vn_norm.text.symbols import all_phonemes, punctuations
from eng_to_ipa import cmu_to_ipa
...
class VPhon:
    ...
    def trans(self, word, dialect, chao, eight, nosuper, glottal, phonemic):
    ...
    def convert(self, word, dialect, chao, eight, nosuper, glottal, phonemic, delimit, try_other=None, tone_end=False):
        ...
        try:
            (ons, gli, nuc, cod, ton) = self.trans(
                word, dialect, chao, eight, nosuper, glottal, phonemic)
            if None in (ons, gli, nuc, cod, ton):
                seq = try_other(word) if try_other is not None else '['+word+']'
            elif tone_end:
                seq = delimit + \
                    delimit.join(filter(None, (ons, gli, nuc, cod, ton)))+delimit
            else:
                seq = delimit + \
                    delimit.join(filter(None, (ons, gli, nuc, ton, cod)))+delimit
        except TypeError:
            pass

        return seq

    def remove_unknown(self, words: str):
        # skip
        return words
        # result = []
        # for c in words:
        #     if c in all_phonemes:
        #         result.append(c)
        # return ''.join(result)

    def convert_en(self, words: str, g2p = None, delimit=None):
        if(delimit is None):
        ...
        for item in result:
            if item == ' ':
                continue
            elif item[0] in punctuations:
                ipa.append(item)
            else:
                ipa_item =  cmu_to_ipa([[item.lower()]])
                ipa.append(ipa_item[0][0])
        return self.remove_unknown(delimit.join(ipa))


    def convert_line_vi(self, line, dialect='o', chao=False, eight=True, nosuper=False, glottal=True, phonemic=False, delimit=None,
                        tokenize=True, tokenize_replace=' ', output_ortho='', try_other=None, tone_end=False, try_other_delimit = ''):
        if(delimit is None):
            delimit = self.delimit
        if line == '\n':
            pass
        else:
            if try_other is None:
                try_other = self.convert_en
            compound = ''
            ortho = ''
            words = line.split()
            # toss len==0 junk
            ...
            for i in range(0, len(words)):
                word = words[i].strip()
                ortho += word
                word_no_punc = word.strip(string.punctuation).lower()
                # if tokenize==true:
                # call this routine for each substring and re-concatenate
                if word_no_punc == '':
                    seq = word
                elif (tokenize and '-' in word) or (tokenize and '_' in word):
                    substrings = re.split(r'(_|-)', word_no_punc)
                    values = substrings[::2]
                    delimiters = substrings[1::2] + ['']
                    ipa = [self.convert(x, dialect, chao, eight, nosuper, glottal,
                                phonemic, delimit, try_other=lambda x: f"{try_other_delimit}{try_other(word)}{try_other_delimit}", tone_end=tone_end).strip() for x in values]
                    if tokenize_replace is not None:
                        seq = tokenize_replace.join(ipa)
                    else:
                        seq = ''.join(v+d for v, d in zip(ipa, delimiters))
                else:
                    seq = self.convert(word_no_punc, dialect, chao, eight, nosuper,
                                glottal, phonemic, delimit, try_other=lambda x: f"{try_other_delimit}{try_other(word)}{try_other_delimit}", tone_end=tone_end).strip()
                # concatenate
                if len(words) >= 2:
                    ortho += ' '
                if i < len(words)-1:
            ...
            if ortho == '':
                ...
                # print orthography?
                if output_ortho:
                    compound = f"{ortho}{output_ortho}{compound}"
                return self.remove_unknown(compound)
...
def main():
    ...
    parser.add_argument("-d", "--dialect", choices=["n", "c", "s", "o"],
                        help="Specify dialect region (Northern, Central, Southern) or spelling pronunciation", type=str.lower)
    parser.add_argument("-c", "--chao", action="store_true",
                        help="Phonetize tones as Chao values")
    parser.add_argument("-g", "--glottal", action="store_true",
                        help="No glottal stops in underlying forms")
    parser.add_argument("-8", "--eight", action="store_true",
                        help="Encode tones as 1-8")
    parser.add_argument("-n", "--nosuper", action="store_true",
                        help="No superscripts anywhere")
    parser.add_argument("-p", "--phonemic", action="store_true",
                        help="Underlying transcriptions after Pham (2006)")
    parser.add_argument("-m", "--delimit", action="store", type=str,
                        help="produce delimited output (bi ca = .b.i.33. .k.a.33.)")
    parser.add_argument("-o", "--ortho", action="store", type=str,
                        dest="output_ortho", help="output orthography as well as IPA (quốc: wok⁴⁵)")
    ...
    parser.add_argument("-i", "--input", action="store",
                        type=str, help="Input the text string")
    ...
    # read from stdin
    fh = args.input.split("\n") if args.input is not None else io.TextIOWrapper(
    ...
    for line in fh:
        if line == '\n':
            ...
            compound = vphon.convert_line_vi(line, dialect, chao, eight, nosuper,
                                       glottal, phonemic, delimit, tokenize=True, output_ortho='|')
            print(compound)
...
if __name__ == '__main__':
    ...
    # main()
    vphon = VPhon()
    # print(remove_unknown('xin_chào12312*'))
    # print(convert_en("hello ball"))
    res = vphon.convert_line_vi(text, try_other_delimit='|')
    print(res)
    # g2p = G2p()
